...
类网易云音乐界面的桌面音乐播放器，使用 Tauri + Vue3 构建客户端，Spring Boot + MySQL 构建后端，通过 NeteaseCloudMusicApi 调用用户 VIP 账号获取歌曲资源。
...
│   │   └── common/           # 公共工具类
│   └── src/main/resources/
└── netease-api-new/          # NeteaseCloudMusicApi (Node.js)
```
...
1. 编辑 `server/src/main/resources/application.yml`，设置 MySQL 密码为 `123456`
2. 创建数据库：
```bash
...
cd netease-api-new && npm install && node app.js
...
```bash
# 克隆代码
git clone https://github.com/1993226871/music-player.git /home/ubuntu
cd /home/ubuntu
...
# 初始化数据库
docker exec music-mysql mysql -u root -p123456 -e "CREATE DATABASE IF NOT EXISTS music_player"
...
- Netease API: http://your-server-ip:3000
...
### 歌曲
- `GET /api/songs/{id}/url` - 获取歌曲播放 URL（带缓存）
...
- 前端 API 基础路径配置在各个 View 组件的 axios 调用中
